(ns gif.decode
  "The data-driven binary decoder. A *grammar* is plain EDN data. `decode`
   walks the grammar over a byte cursor and returns a nested EDN map.
   Self-contained copy of the generic engine kasane.decode provides —
   duplicated deliberately so this repo has no back-dependency on kasane."
  (:require [gif.bytes :as b]))
